...
class ChainStore {
  ...
  getAccountRefsOfKey(key) {
    if (this.get_account_refs_of_keys_calls.has(key))
      ...
      Apis.instance().db_api().exec("get_key_references", [[key]]).then(
        vec_account_id => {
  ...
  getBalanceObjects(address) {
    ...
    if (current === undefined) {
      ...
      Apis.instance().db_api().exec("get_balance_objects", [[address]]).then(
        balance_objects => {
          let set = new Set();
  ...
  fetchObject(id, force = false, autosubscribe = true) {
    ...
    if (result === undefined) {
      ...
      result === true // then we are waiting a response
    )
      return undefined;
  ...
  getAccount(name_or_id, autosubscribe = true) {
    ...
    if (ChainValidation.is_object_id(name_or_id)) {
      ...
      if (
        account_id === undefined // then no query, fetch it
      )
        return this.fetchFullAccount(name_or_id, autosubscribe);
  ...
  getObjectsByVoteIds(vote_ids) {
    ...
    if (missing.length) {
      ...
      Apis.instance().db_api().exec("lookup_vote_ids", [missing]).then(
        vote_obj_array => {
